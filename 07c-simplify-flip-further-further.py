...
output_unique = Path("data/8-knotoids-unique-up-to-flip.txt")  # save PD codes of knotoids
output_nonunique = Path("data/8-knotoids-groups-up-to-flip.txt")  # save PD codes of knotoids
...
for group in tqdm(data):
    ...
    eq = kp.reduce_equivalent_diagrams(group, depth=4, flype=True)
